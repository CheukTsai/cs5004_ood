...
public class MobileApp implements Comparable<MobileApp> {
  ...
  public int compareTo(MobileApp o) {
    ...
    Integer totalDownloadsToCompare = 0;

    for (String OS : this.getSupportedOS().keySet()) {
      totalDownloads += this.getSupportedOS().get(OS);
    }

    for (String OS : o.getSupportedOS().keySet()) {
      totalDownloadsToCompare += o.getSupportedOS().get(OS);
    }

//    if (totalDownloads < totalDownloadsToCompare) {
//      return -1;
//    } else if (totalDownloads > totalDownloadsToCompare) {
//      return 1;
//    } else {
//      return 0;
//    }
    return totalDownloads.compareTo(totalDownloadsToCompare);
  }
